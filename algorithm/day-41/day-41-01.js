...
function solution(record) {
  ...
  for (let i = 0; i < record.length; i++) {
    let status = record[i].split(" ")[0];

    if (status === "Enter") answer.push(user[record[i].split(" ")[1]] + "님이 들어왔습니다.");
    if (status === "Leave") answer.push(user[record[i].split(" ")[1]] + "님이 나갔습니다.");
  }
...
function solution(record) {
  ...
  for (let idx in answer) {
    answer[idx] = users[answer[idx].uid] + (answer[idx].action === "Enter" ? "님이 들어왔습니다." : "님이 나갔습니다.");
  }
...
function solution(record) {
  ...
  const answer = record.reduce((acc, cur) => {
    if (cur[0] !== "Change") {
      acc.push(`${users[cur[1]]}님이 ` + (cur[0] === "Enter" ? "들어왔습니다." : "나갔습니다."));
    }
