  ...
  for (let i = 0; i < moves.length; i++) {
    for (let j = 0; j < board.length; j++) {
      ...
      if (doll !== 0) {
        ...
        // 바구니에 인형을 넣으려고 할 때, 바구니의 맨 위의 인형이 현재 넣으려는 인형과 같다면, 바구니 맨 위 인형 제거
        ...
        // 6. 인형을 뽑았다면, 같은 위치에 대한 크레인의 동작 종료
        break;
      }
    }
  }
...
function solution(board, moves) {
  ...
  moves.forEach((move) => {
    board.forEach((location) => {
      ...
      // false일 때만 forEach 내부에 로직이 동작하도록
      // why? => forEach 내부에선 break를 사용할 수 없기때문에
      let pick = false;
      // console.log(move, location)
      const doll = location[move - 1];
      // console.log(move, location, doll)
